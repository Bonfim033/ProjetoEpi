...
import 'package:projeto_epi/constrain/url.dart';
...
class ValidarSenha extends ChangeNotifier {
  ...
  void validadePassword(String password) {
    ...
    if (password.length < 8) {
      _msgError = 'Minimo 8 digitos';
    } else if (!password.contains(RegExp(r'[a-z]'))) {
      _msgError = 'Pelo menos uma letra minuscula';
    } else if (!password.contains(RegExp(r'[A-Z]'))) {
      _msgError = 'Pelo menos uma letra maiuscula';
    } else if (!password
        .contains(RegExp(r'[!@#$%^&*()_+\-=\[\]{};:\|,.<>\/?]'))) {
      _msgError = 'Pelo menos um caracter especial';
    } else if (!password.contains(RegExp(r'[0-9?]'))) {
      _msgError = 'Pelo menos um numero';
    }
  ...
  Future createUser(String email, String password, int cpf) async {
    ...
    if (response.statusCode == 200) {
      _carregando = true;
      _msgErrorApi = 'Usuario Cadastrado com sucesso';
      notifyListeners();
    } else {
